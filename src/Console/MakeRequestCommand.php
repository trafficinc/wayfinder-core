...
final class MakeRequestCommand implements Command
{
    ...
    public function handle(array $arguments = []): int
    {
        ...
        if (! is_string($name) || trim($name) === '') {
            fwrite(STDERR, "Usage: php wayfinder make:request <name>\n");
        ...
        if (! is_dir($directory) && ! mkdir($directory, 0777, true) && ! is_dir($directory)) {
            fwrite(STDERR, sprintf("Unable to create request directory [%s].\n", $directory));

            return 1;
        }

        if (file_exists($target)) {
            fwrite(STDERR, sprintf("Request [%s] already exists.\n", $relative));
...
PHP;
        ...
        if (file_put_contents($target, $template . "\n") === false) {
            fwrite(STDERR, sprintf("Unable to write request [%s].\n", $relative));

            return 1;
        }

        fwrite(STDOUT, sprintf("Created request: %s\n", $relative));
    ...
    private function normalizeClassName(string $name): string
    {
        ...
        if ($normalized === []) {
            throw new \InvalidArgumentException('Request name must contain letters or numbers.');
        }

        $last = array_pop($normalized);

        if (! str_ends_with($last, 'Request')) {
            $last .= 'Request';
        }
